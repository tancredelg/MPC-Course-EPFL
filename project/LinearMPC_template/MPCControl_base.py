...
import mpt4py.polyhedron as mpt
...
class MPCControl_base:
    ...
    def _compute_terminal_components(self):
        """Compute LQR P matrix and Terminal Invariant Set X_f"""
        
        # 1. LQR for Terminal Cost P and gain K
        # Solve Discrete Algebraic Riccati Equation
        P = scipy.linalg.solve_discrete_are(self.A, self.B, self.Q, self.R)
        self.P = P
        
        # Compute LQR gain K = (R + B'PB)^-1 B'PA
        R_inv = np.linalg.inv(self.R + self.B.T @ P @ self.B)
        K = R_inv @ (self.B.T @ P @ self.A)
        ...
        # Combine
        H_set = np.vstack([H_x, H_u])
        h_set = np.hstack([h_x, h_u])
        
        # Create Polyhedron and compute Maximum Output Admissible Set (MOAS) / Invariant Set
        # Note: We use the Delta state for the set
        poly = mpt.Polyhedron(A=H_set, b=h_set)
        
        # Compute invariant set (O_inf)
        # We need a robust invariant set calculation. 
        # mpt4py might act differently depending on backend, but standard call is:
        self.X_f = poly.invariant_set(dynamics=A_cl)
